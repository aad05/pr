...
const Kutubxona = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_MAIN_URL}/user/course/courseParts?page=${paginationData}&limit=5`,
        {
  ...
  const postD = () => {
    if (postData.name && postData.description) {
      ...
      axios
        .post(
          'https://coursesnodejs.herokuapp.com/employee/course',
          {
            name: postData.name,
            imgUrl: imgUrl || 'img/img-a463268af6f271bc3adac0871d505b4a.jpg',
            description: postData.description || 'Description empty',
          },
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
      ...
      axios
        .put(
          `https://coursesnodejs.herokuapp.com/employee/course`,
          {
            ...
            description: selectData.description || 'Description empy',
            imgUrl: imgUrl || selectData.imgUrl,
          },
          {
            headers: {
          ...
          return (
            ...
            >
              <Wrapper.CardInside>
                <Wrapper.Meta
                  onClick={() =>
                    navigate(`/courses/${value.name}/${value._id}`)
                  }
                  title={value.name}
                />
              </Wrapper.CardInside>
